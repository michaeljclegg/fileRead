<template>
  <div
    :class="[
      'flex items-center p-3 bg-slate-800/70 border rounded-lg transition-all duration-300',
      statusColorClass,
    ]"
  >
    ...
    <div class="grow min-w-0">
      ...
      <p
        v-if="status === UploadStatus.ERROR && error"
    ...
    <div class="shrink-0 ml-4 flex items-center gap-2">
      ...
      <button
        v-if="status === UploadStatus.PENDING || status === UploadStatus.ERROR"
        @click="$emit('remove')"
        class="ml-2 p-1 text-slate-400 hover:text-red-400 hover:bg-slate-700 rounded-full transition-colors"
        title="Remove file"
      >
...
const statusColorClass = computed(() => {
  switch (props.status) {
    ...
    case UploadStatus.PROCESSING:
      return "border-purple-700 bg-purple-900/20"; // Mapped CONVERTING to PROCESSING
    case UploadStatus.UPLOADING:
      return "border-sky-700 bg-sky-900/20";
    case UploadStatus.SUCCESS:
      return "border-green-700 bg-green-900/20";
    case UploadStatus.ERROR:
      return "border-red-700 bg-red-900/20"; // Mapped FAILED to ERROR
    default:
      return "border-slate-700";
  }
});
...
const statusIconClass = computed(() => {
  switch (props.status) {
    ...
    case UploadStatus.PROCESSING:
      return "text-purple-400 animate-spin";
    case UploadStatus.UPLOADING:
